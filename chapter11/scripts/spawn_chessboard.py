...
if __name__ == '__main__':
    ...
    delete_model("chess_board")
    s = rospy.ServiceProxy("gazebo/spawn_sdf_model", SpawnModel)
    orient = Quaternion(*tf.transformations.quaternion_from_euler(0, 0, 0))
    board_pose = Pose(Point(0.25, 1.39, 0.90), orient)
    unit = 0.05
    with open("src/chapter11/models/chess_board.sdf", "r") as f:
        board_xml = f.read()
    with open("src/chapter11/models/chess_piece.sdf", "r") as f:
        piece_xml = f.read()
    print(s("chess_board", board_xml, "", board_pose, "world"))
    for row in [0, 1, 6, 7]:
